...
**目标:** 在 rust-agent-middlewares/src/plugin/ 下实现兼容 Claude Code 的插件核心基础设施——类型定义、配置读写、marketplace 发现与缓存、插件安装管理、插件加载器与中间件。
...
- 关键设计决策：`PluginManifest.mcp_servers` 字段直接复用现有 `McpServerConfig` 类型（`rust-agent-middlewares/src/mcp/config.rs:17`），通过 `use crate::mcp::McpServerConfig` 引用
...
- [x] 检查现有依赖中是否包含 `serde_yaml` 或 `gray_matter`
  - `grep -E "(serde_yaml|gray_matter)" rust-agent-middlewares/Cargo.toml`
  - 预期: 至少有一个已存在（Task 4 使用 gray_matter 解析 frontmatter）
- [x] 检查 `dirs_next` 依赖已存在
  - `grep "dirs_next" rust-agent-middlewares/Cargo.toml`
  - 预期: 找到依赖声明
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/plugin/types.rs`
- 新建: `rust-agent-middlewares/src/plugin/config.rs`
- 新建: `rust-agent-middlewares/src/plugin/mod.rs`
- 修改: `rust-agent-middlewares/src/lib.rs`

**执行步骤:**
- [x] 创建 `rust-agent-middlewares/src/plugin/` 目录
  - `mkdir -p rust-agent-middlewares/src/plugin/`

- [x] 编写插件核心类型定义文件 `types.rs`
  - 位置: `rust-agent-middlewares/src/plugin/types.rs`（新建文件）
  - 文件头部引入依赖：
    ```rust
...
- [x] 编写配置读写模块 `config.rs`
  - 位置: `rust-agent-middlewares/src/plugin/config.rs`（新建文件）
  - 文件头部引入依赖：
    ```rust
...
- [x] 编写模块入口文件 `mod.rs`
  - 位置: `rust-agent-middlewares/src/plugin/mod.rs`（新建文件）
  - 内容：
    ```rust
    pub mod config;
    pub mod types;
    ...
    ```
  - 原因: 与现有 `mcp/mod.rs`（`rust-agent-middlewares/src/mcp/mod.rs`）结构一致——子模块声明 + 公共类型重导出

- [x] 在 `lib.rs` 中注册 plugin 模块
  - 位置: `rust-agent-middlewares/src/lib.rs`，在 `pub mod skills;`（L32）之后、`pub mod tools;`（L33）之前插入
  - 插入内容：
    ```rust
    pub mod plugin;
    pub use plugin::{
  ...
  - 在 prelude 模块（L59 的 `pub mod prelude {` 内部）中，在 `pub use crate::skills::` 之前插入：
    ...
    ```
  - 原因: 遵循现有模块注册模式（`pub mod mcp;` + `pub use mcp::{...};`），prelude 导出确保外部 crate 可通过 `use rust_agent_middlewares::prelude::*;` 获取插件类型

- [x] 为 `types.rs` 核心类型编写单元测试
  - 测试文件: `rust-agent-middlewares/src/plugin/types.rs`（文件底部 `#[cfg(test)] mod tests { ... }` 块）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::types::tests`
  - 预期: 所有测试通过

- [x] 为 `config.rs` 配置读写函数编写单元测试
  - 测试文件: `rust-agent-middlewares/src/plugin/config.rs`（文件底部 `#[cfg(test)] mod tests { ... }` 块）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::config::tests`
  - 预期: 所有测试通过
...
- [x] 验证 types.rs 单元测试全部通过
  - `cargo test -p rust-agent-middlewares --lib -- plugin::types::tests 2>&1 | tail -10`
  - 预期: 所有 test 以 `ok` 结尾，无 failure
- [x] 验证 config.rs 单元测试全部通过
  - `cargo test -p rust-agent-middlewares --lib -- plugin::config::tests 2>&1 | tail -10`
  - 预期: 所有 test 以 `ok` 结尾，无 failure
- [x] 验证 plugin 模块已注册到 lib.rs
  - `grep -n "pub mod plugin" rust-agent-middlewares/src/lib.rs`
  - 预期: 输出包含 `pub mod plugin;` 的行号
- [x] 验证 prelude 导出包含插件类型
  - `grep -c "PluginManifest" rust-agent-middlewares/src/lib.rs`
  - 预期: 输出 >= 2（mod 级导出 + prelude 导出）
- [x] 验证 McpServerConfig 复用关系正确
  - `grep "use crate::mcp::McpServerConfig" rust-agent-middlewares/src/plugin/types.rs`
  - 预期: 输出该行，确认引用路径正确
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/plugin/marketplace.rs`
- 修改: `rust-agent-middlewares/src/plugin/mod.rs`
- 修改: `rust-agent-middlewares/Cargo.toml`（新增 flate2 + tar 依赖）

**执行步骤:**
- [x] 在 `Cargo.toml` 中新增 NPM tarball 解压依赖
  - 位置: `rust-agent-middlewares/Cargo.toml`，在 `regex = "1"`（L38）行之后插入
  - 插入内容：
    ```toml
    flate2 = "1"
    tar = "0.4"
...
- [x] 实现 `MarketplaceError` 错误类型和状态枚举
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`（新建文件）
  - 文件头部引入：
    ```rust
...
- [x] 实现 `find_marketplace_json` 和 `read_manifest_from_path` — 通用辅助函数
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`，在类型定义之后
...
- [x] 实现 `fetch_github` — GitHub 源拉取
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`，在辅助函数之后
...
- [x] 实现 `fetch_url` — URL 源拉取
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`，在 `fetch_github` 之后
...
- [x] 实现 `read_file` 和 `read_directory` — 本地源读取
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`，在 `fetch_url` 之后
...
- [x] 实现 `fetch_npm` — NPM 源拉取
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`，在本地源函数之后
...
- [x] 实现 `MarketplaceManager` 结构体和 `new` 方法
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`，在所有拉取函数之后
  - 定义结构体:
    ```rust
...
- [x] 实现 `MarketplaceManager::init` — 初始化入口
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`，在 `new` 之后
...
- [x] 实现 `MarketplaceManager::try_load_cache` — 缓存加载分发
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`，在 `init` 之后
...
- [x] 实现 `MarketplaceManager::spawn_refresh` — 后台异步刷新
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`，在 `try_load_cache` 之后
...
- [x] 实现 `MarketplaceManager` 的查询方法
  - 位置: `rust-agent-middlewares/src/plugin/marketplace.rs`，在 `spawn_refresh` 之后
...
- [x] 在 `mod.rs` 中注册 marketplace 模块并导出公共 API
  - 位置: `rust-agent-middlewares/src/plugin/mod.rs`，在 `pub mod types;` 之后插入
...
- [x] 为 GitHub 拉取和辅助函数编写单元测试
  - 测试文件: `rust-agent-middlewares/src/plugin/marketplace.rs`（文件底部 `#[cfg(test)] mod tests { ... }` 块）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::marketplace::tests::test_find`
  - 预期: 所有测试通过

- [x] 为本地源读取编写单元测试
  - 测试文件: `rust-agent-middlewares/src/plugin/marketplace.rs`（内联测试模块）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::marketplace::tests::test_read`
  - 预期: 所有测试通过
...
- [x] 为 URL 拉取逻辑编写单元测试
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::marketplace::tests::test_url`
...
- [x] 为 MarketplaceManager 编写单元测试
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::marketplace::tests::test_manager`
  - 预期: 所有测试通过
...
- [x] 验证所有 marketplace 单元测试通过（非 integration）
  - `cargo test -p rust-agent-middlewares --lib -- plugin::marketplace::tests 2>&1 | tail -15`
  - 预期: 所有 test 以 `ok` 结尾，标记 `ignore` 的 integration test 显示为 `ignored`
- [x] 验证新增依赖声明存在
  - `grep -E "^(flate2|tar)" rust-agent-middlewares/Cargo.toml`
  - 预期: 输出包含 `flate2 = "1"` 和 `tar = "0.4"` 两行
- [x] 验证 mod.rs 导出 marketplace 类型
  - `grep "MarketplaceManager" rust-agent-middlewares/src/plugin/mod.rs`
  - 预期: 输出包含 `MarketplaceManager` 的行
- [x] 验证 lib.rs 和 prelude 导出更新
  - `grep -c "MarketplaceManager" rust-agent-middlewares/src/lib.rs`
  - 预期: 输出 >= 2（mod 级导出 + prelude 导出）
- [x] 验证 Task 1 测试无回归
  - `cargo test -p rust-agent-middlewares --lib -- plugin::types::tests 2>&1 | tail -5`
  - 预期: 所有测试通过
- [x] 验证 Task 1 config 测试无回归
  - `cargo test -p rust-agent-middlewares --lib -- plugin::config::tests 2>&1 | tail -5`
  - 预期: 所有测试通过
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/plugin/installer.rs`
- 修改: `rust-agent-middlewares/src/plugin/mod.rs`（导出 installer 模块公共 API）

**执行步骤:**

- [x] 创建 installer.rs 并定义 InstallerError 错误类型
  - 位置: `rust-agent-middlewares/src/plugin/installer.rs` 文件顶部
...
- [x] 实现 `install_plugin` 核心函数
  - 位置: `rust-agent-middlewares/src/plugin/installer.rs`
...
- [x] 实现 `uninstall_plugin` 函数
  - 位置: `rust-agent-middlewares/src/plugin/installer.rs`，紧跟 `install_plugin` 之后
...
- [x] 实现 `update_plugin` 函数
  - 位置: `rust-agent-middlewares/src/plugin/installer.rs`，紧跟 `uninstall_plugin` 之后
...
- [x] 实现 `check_updates` 批量检查函数
  - 位置: `rust-agent-middlewares/src/plugin/installer.rs`，紧跟 `update_plugin` 之后
  - 定义返回类型:
    ```rust
    #[derive(Debug, Clone)]
...
- [x] 实现 `update_enabled_plugins` 和 `remove_from_enabled_plugins` 辅助函数
  - 位置: `rust-agent-middlewares/src/plugin/installer.rs`，在公共函数之后、单元测试之前
...
- [x] 实现 `copy_dir_recursive` 目录递归复制辅助函数
  - 位置: `rust-agent-middlewares/src/plugin/installer.rs`，在辅助函数区域
...
- [x] 为 installer 核心逻辑编写单元测试
  - 测试文件: `rust-agent-middlewares/src/plugin/installer.rs` 底部 `#[cfg(test)] mod tests`
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::installer::tests`
  - 预期: 所有测试通过

**检查步骤:**
- [x] 验证 installer.rs 文件存在且包含所有公共函数
  - `grep -c "pub async fn" rust-agent-middlewares/src/plugin/installer.rs`
  - 预期: 输出 >= 4（install_plugin, uninstall_plugin, update_plugin, check_updates）
- [x] 验证 InstallerError 使用 thiserror 派生
  - `grep "thiserror" rust-agent-middlewares/src/plugin/installer.rs`
  - 预期: 匹配到 `#[derive(thiserror::Error)]` 或 `#[derive(Error)]` 配合 `use thiserror::Error`
- [x] 验证 mod.rs 正确 re-export installer 公共 API
  - `grep "installer" rust-agent-middlewares/src/plugin/mod.rs`
  - 预期: 包含 `pub mod installer;` 和 `pub use installer::`
- [x] 验证单元测试编译通过
  - `cargo test -p rust-agent-middlewares --lib -- plugin::installer::tests --no-run 2>&1 | tail -3`
  - 预期: 输出包含 "Finished" 且无编译错误
- [x] 验证所有单元测试通过
  - `cargo test -p rust-agent-middlewares --lib -- plugin::installer::tests 2>&1 | tail -5`
  - 预期: 所有测试通过，无失败
...
[修改原因] — Task 1-3 完成了类型定义、配置读写和安装管理，但缺少"从已安装插件提取资源并注入 agent 运行时"的核心加载逻辑。当前 `run_universal_agent`（`rust-agent-tui/src/app/agent.rs`）中的中间件链（L246-L268）硬编码了固定的 SkillsMiddleware、McpMiddleware、SubAgentMiddleware，未纳入插件提供的额外资源。
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/plugin/loader.rs`
- 新建: `rust-agent-middlewares/src/plugin/middleware.rs`
- 修改: `rust-agent-middlewares/src/plugin/mod.rs`（导出新模块公共 API）
- 修改: `rust-agent-middlewares/src/lib.rs`（prelude 导出新增类型）

**执行步骤:**

- [x] 实现 `CommandProvider` trait 和 `CommandEntry`/`CommandSource` 类型
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`（新建文件）
  - 文件头部引入：
    ```rust
...
- [x] 实现 `CommandFrontmatter` 和 `parse_command_md` — 插件命令 Markdown 解析
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 CommandProvider 定义之后
...
- [x] 实现 `LoadedPlugin` 结构体 — 插件加载结果
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 parse_command_md 之后
  - 定义结构体：
    ```rust
...
- [x] 实现 `load_manifest` — 单个插件清单加载
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 LoadedPlugin 之后
...
- [x] 实现 `extract_commands` — 从清单中提取命令列表
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 load_manifest 之后
...
- [x] 实现 `extract_skills_paths` — 从清单中提取 skills 目录路径
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 extract_commands 之后
...
- [x] 实现 `extract_agents_paths` — 从清单中提取 agents 目录路径
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 extract_skills_paths 之后
...
- [x] 实现 `extract_mcp_servers` — 从清单中提取 MCP 服务器配置
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 extract_agents_paths 之后
...
- [x] 实现 `load_plugins` — 批量加载已安装插件
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 extract 函数之后
...
- [x] 实现 `load_enabled_plugins` — 仅加载已启用插件（读取 enabledPlugins 列表）
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 load_plugins 之后
...
- [x] 实现 `PluginCommandProvider` — CommandProvider 的插件实现
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 load_enabled_plugins 之后
  - 定义结构体:
    ```rust
...
- [x] 实现 `merge_plugin_mcp_servers` — 将插件 MCP 服务器合并到现有配置
  - 位置: `rust-agent-middlewares/src/plugin/loader.rs`，在 PluginCommandProvider 之后
...
- [x] 实现 `PluginMiddleware` — Middleware trait 实现
  - 位置: `rust-agent-middlewares/src/plugin/middleware.rs`（新建文件）
  - 文件头部引入：
    ```rust
...
- [x] 在 mod.rs 中注册 loader 和 middleware 模块并导出公共 API
  ...
  - 在 `pub use` 重导出区域追加:
    ...
    ```
  - 在 `rust-agent-middlewares/src/lib.rs` 的 `pub use plugin::{ ... };` 列表末尾追加: `CommandEntry, CommandProvider, CommandSource, LoadedPlugin, LoaderError, PluginCommandProvider, PluginMiddleware`
  - 在 `rust-agent-middlewares/src/lib.rs` 的 `pub mod prelude { ... }` 的 `pub use crate::plugin::{ ... };` 列表末尾同样追加
  - 原因: 遵循现有模块注册和导出模式，确保 Task 5 和 Task 6 可通过 `rust_agent_middlewares::prelude::*` 获取类型

- [x] 为 `parse_command_md` 和 frontmatter 解析编写单元测试
  - 测试文件: `rust-agent-middlewares/src/plugin/loader.rs` 底部 `#[cfg(test)] mod tests`
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::loader::tests::test_parse`
  - 预期: 所有测试通过

- [x] 为 `extract_commands` 编写单元测试
  - 测试文件: `rust-agent-middlewares/src/plugin/loader.rs`（内联测试模块）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::loader::tests::test_extract_commands`
  - 预期: 所有测试通过
...
- [x] 为 `extract_skills_paths` 和 `extract_agents_paths` 编写单元测试
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::loader::tests::test_extract_skills`
  - 预期: 所有测试通过
...
- [x] 为 `extract_mcp_servers` 和 `merge_plugin_mcp_servers` 编写单元测试
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::loader::tests::test_extract_mcp`
  - 预期: 所有测试通过
...
- [x] 为 `load_plugins` 和 `load_enabled_plugins` 编写单元测试
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::loader::tests::test_load`
  - 预期: 所有测试通过
...
- [x] 为 `PluginCommandProvider` 编写单元测试
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::loader::tests::test_plugin_command_provider`
  - 预期: 所有测试通过

- [x] 为 `PluginMiddleware` 编写单元测试
  - 测试文件: `rust-agent-middlewares/src/plugin/middleware.rs` 底部 `#[cfg(test)] mod tests`
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- plugin::middleware::tests`
  - 预期: 所有测试通过
...
- [x] 验证 loader.rs 所有单元测试通过
  - `cargo test -p rust-agent-middlewares --lib -- plugin::loader::tests 2>&1 | tail -15`
  - 预期: 所有 test 以 `ok` 结尾，无 failure
- [x] 验证 middleware.rs 所有单元测试通过
  - `cargo test -p rust-agent-middlewares --lib -- plugin::middleware::tests 2>&1 | tail -10`
  - 预期: 所有 test 以 `ok` 结尾，无 failure
- [x] 验证 mod.rs 正确 re-export loader 和 middleware 公共 API
  - `grep -E "(loader|middleware|CommandProvider|PluginMiddleware)" rust-agent-middlewares/src/plugin/mod.rs`
  - 预期: 输出包含 `pub mod loader;`、`pub mod middleware;`、`pub use loader::`、`pub use middleware::PluginMiddleware`
- [x] 验证 lib.rs 和 prelude 导出更新
  - `grep -c "PluginMiddleware" rust-agent-middlewares/src/lib.rs`
  - 预期: 输出 >= 2（mod 级导出 + prelude 导出）
  - `grep -c "CommandProvider" rust-agent-middlewares/src/lib.rs`
  - 预期: 输出 >= 2
- [x] 验证 gray_matter 被复用（无新增 serde_yaml 以外依赖）
  - `grep "gray_matter" rust-agent-middlewares/src/plugin/loader.rs`
  - 预期: 输出包含 `use gray_matter`，确认复用现有依赖
- [x] 验证 McpServerConfig 复用关系正确
  - `grep "use crate::mcp::McpServerConfig" rust-agent-middlewares/src/plugin/loader.rs`
  - 预期: 输出该行，确认引用路径正确
- [x] 验证 Task 1-3 测试无回归
  - `cargo test -p rust-agent-middlewares --lib -- plugin:: 2>&1 | tail -20`
  - 预期: 所有测试通过，无回归
- [x] 验证全量 middlewares 测试无回归
  - `cargo test -p rust-agent-middlewares --lib 2>&1 | tail -10`
  - 预期: 所有测试通过，无回归
...
**前置条件:**
- 构建环境: `cargo build -p rust-agent-middlewares` 成功
...
3. 验证 marketplace 拉取和缓存
   - `cargo test -p rust-agent-middlewares --lib -- plugin::marketplace 2>&1 | tail -15`
   - 预期: GitHub/URL/local helper 测试通过
   - 失败排查: 检查 Task 2 marketplace.rs 的缓存目录创建和 manifest 解析

4. 验证安装/卸载流程
   - `cargo test -p rust-agent-middlewares --lib -- plugin::installer 2>&1 | tail -15`
   - 预期: 12 个安装/卸载/更新测试全部通过
   - 失败排查: 检查 Task 3 installer.rs 的文件复制和 installed_plugins.json 写入

5. 验证插件加载和资源提取
   - `cargo test -p rust-agent-middlewares --lib -- plugin::loader 2>&1 | tail -15`
   - 预期: 7 组加载/提取测试全部通过
